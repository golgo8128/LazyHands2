...
curated_peaks_zscores = ...
    arrayfun(@(tmpmzidx, tmpmtidx) dat2d_z_smoothed(tmpmzidx, tmpmtidx), ...
...
dat2d_z(dat2d_z_smoothed < 3.89) = 0; %% <--- Important!?
...
dat2d_z_smoothed_sel = dat2d_z_smoothed(ephemat_vis.range_l_m > 50, 4 <=ephemat_vis.range_l_n & ephemat_vis.range_l_n < 25);

% image(dat2d_z(range_l_m > 50 , range_l_n < 25))

sum(dat2d_z_sel > 5.00, [1, 2]) / sum(dat2d_z_sel > -inf, [1, 2]) % 1.96
sum(dat2d_z_smoothed_sel > 5.00, [1, 2]) / sum(dat2d_z_smoothed_sel > -inf, [1, 2]) % 1.96
sum((mat_dir_shift_sum_iter1(dat2d_z_smoothed_sel > 3.89, 2^2 + 2^3, 1+10) + ...
     mat_dir_shift_sum_iter1(dat2d_z_smoothed_sel > 3.89, 2^0 + 2^1, 1+1)) > 0, [1, 2]) / sum(dat2d_z_sel > -inf, [1, 2]) % 1.96
