...
# For unit test, rsunit_test_PeakGrps <- T and source it.

PeakGrps <-
  setRefClass("PeakGrps",
    fields = list(
      pkgrp_list = "list"
      )) # "ANY" as generic class name?


PeakGrps$methods(initialize =
    function(){
      
    })

PeakGrps$methods(add_peakgrp =
    function(ipkgrp){
      .self$pkgrp_list <- c(.self$pkgrp_list, ipkgrp)
    })


if(exists("rsunit_test_PeakGrps") &&
   rsunit_test_PeakGrps){
  ...
  source.RS("MetabDatAnalyses/TreatPeaks/PeakGrpSingle1_1.R")
  ...
  tmp_pkgrps <- PeakGrps()
  tmp_pkgrps$add_peakgrp(tmp_pkgrpsing)
