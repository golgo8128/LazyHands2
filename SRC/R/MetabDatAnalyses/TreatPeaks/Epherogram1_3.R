...
source.RS("Usefuls1/FindPeakSimple1_2.R")
...
EPherogram$methods(find_bulk_peaks = 
  function(ictrl_range_mt = c(2,5)*60,
           ipeak_width_range = c(0.02, 5) * 60, # Can be Inf, # This can be ratio of peak top height and width
    ...
    if(!is.na(ctrl_sd) && ctrl_sd > 0){
      pk_info <- 
        FindPeakSimple(
          ...
          split_peak_drop_rate_thres     = split_peak_drop_rate_thres,
          split_peak_rerise_rate_thres   = split_peak_rerise_rate_thres,
          # split_peak_top_update_ct_thres = split_peak_top_update_ct_thres,
          split_peak_rerise_factor_thres = split_peak_rerise_factor_thres)
      
      for(i in 1:nrow(pk_info$get_peak_poss())){
        pos_start <- pk_info$get_peak_poss()[ i, "start" ]
        pos_end   <- pk_info$get_peak_poss()[ i, "end" ]
        mt_start  <- imt[ pos_start ]
        mt_end    <- imt[ pos_end ]
        mt_diff   <- mt_end - mt_start
        if(ipeak_width_range[1] <= mt_diff && mt_diff < ipeak_width_range[2]){
          pk_single <- PeakSingle(.self, pos_start, pos_end)
          pk_single$h$zscore  <- pk_info$get_peak_poss()[ i, "zscore" ]
          pk_single$h$height_sum <-
            sum((iintsty[ pos_start : pos_end ] - ctrl_mean))
          .self$add_peak(pk_single)
          # print(c(ctrl_mean, ctrl_sd))
        }
      }
    }
