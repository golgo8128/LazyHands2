...
RefSampPairSet$methods(
  plot_peak_in_ephe =
    function(imetabid = "",
      ...
      ### Determines axes ranges and plot title ###
      ...
      if(imetabid != ""){
        ...
        if(!is.null(ephe_ref)){
          ...
          plot_title     <- sprintf("[ m/z: %.4f ] ~%.4f", ephe_ref$mz, imz)
        } else {
          peak_ref_range <- NULL
          peak_ref_top   <- NULL
          plot_title     <- ""
      ...
      ### Determines axes ranges and plot title (End) ###
      ...
      if(length(peak_intsty_tops)){
        ...
        # In case imetabid is specified but failed to generate electropherogram.
        if(is.null(ephe_ref) && !is.na(mz)){
          ephe_ref <- .self$ref$find_ephe_mz(mz, imax_diff_mz, ippm)
        }
        ...
        for(i in 1:length(.self$smp_l)){
          ...
          if(is.null(ephe_smp) && !is.na(mz)){
            ephe_smp <- smp$find_ephe_mz(mz, imax_diff_mz, ippm)
          }
...
if(exists("rsunit_test_RefSampPairSet") &&
  ...
  tmp_ref1$annotlist$reg_mz("P003", 116.07)
  tmp_ref1$annotlist$reg_mt("P003", 860)
